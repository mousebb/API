...
func (r *Router) HandleRoute(method string, pattern string, handler http.Handler) {
	r.Router.Handle(method, pattern, wrapHandler(handler))
}
...
func New() *Router {
	...
	for _, route := range routes {
		...
		case SHOPIFY_ACCOUNT_LOGIN_ENDPOINT:
			r.HandleRoute(route.Method, route.Pattern, common.Append(middleware.ShopifyAccountLogin).ThenFunc(route.Handler))
